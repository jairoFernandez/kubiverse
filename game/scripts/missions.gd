...
const PROD := [
	{"id": "p_census", "title": "Census", "title_es": "Censo",
		...
		"cmd": "kubectl get nodes -o wide && kubectl get pods -A --no-headers | wc -l"},
	...
	{"id": "p_bottleneck", "title": "Bottleneck hunt", "title_es": "Caza del cuello de botella",
		...
		"learn": "When one node is much fuller than the rest, new pods pile up on the others or stay Pending. Spread them with requests that match reality, anti-affinity or topology spread constraints.",
		"learn_es": "Cuando un nodo está mucho más lleno que el resto, los pods nuevos se amontonan en los demás o se quedan Pending. Repártelos con requests realistas, anti-afinidad o topology spread constraints.",
		"cmd": "kubectl describe nodes | grep -A8 'Allocated resources'"},
