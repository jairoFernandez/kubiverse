...
func close_modals() -> bool:
	...
	if editor.visible:
		...
		return true
	for p in [_confirm_panel, _upd_panel, _build_panel, _guide_panel, _map_panel, _logs_panel, _view_panel, _vol_panel, _alarm_panel, _search_panel, _legend, kubi, watch]:
		if p.visible:
			p.visible = false
			_sync_view()
			return true
...
func _update_target_title(target: String) -> String:
	...
	return tr("The bridge (k8s-bridge)")
