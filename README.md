...
- **Storage you can see**: the plant has a **storage yard** east of the energy plant: a factory per StorageClass (the default one with a star) and the PersistentVolumes as big tanks (green Bound, blue Available, orange Released, red Failed) filled to their **real use** when Prometheus has the kubelet's volume stats. Pipes run from the yard to the halls whose claims live there. In each hall, every PersistentVolumeClaim is a tank piped along the floor to the pods that mount it; its inspector says size, use, class, access modes and, when it's stuck, why (a StorageClass that doesn't exist and which ones do, `WaitForFirstConsumer`, the provisioner). A volume over 90% full raises an alarm, and so does a Released or Failed PV.
- **Secrets and ConfigMaps**: each hall has a **vault** (a steel floor with hazard stripes): a **safe** per Secret, with a padlock the colour of its type (gold ribbon for TLS, a blue key for registry credentials) and a **filing cabinet** per ConfigMap with a drawer per key the pods read by name. Cables go to the pods that use them (as files, env variables, `envFrom` or to pull images). A Secret or ConfigMap that pods reference but that doesn't exist is a **red ghost** and an alarm (those pods can't start); one nobody uses gathers dust. **Values are never read**: what pods use comes from their own specs, whether a Secret exists from a metadata-only list with every annotation dropped as it arrives (kubectl's last-applied annotation would carry the data), and nothing about them reaches the AI. In the Helm chart, listing Secret names is off by default (`secrets.listNames`), because RBAC can't grant "list" without "read".
