...
bridge:
	cd bridge && go build -ldflags="-X main.version=$(BRIDGE_VERSION)" -o bin/k8s-bridge .
...
bridge-all:
	cd bridge && for t in darwin/arm64 darwin/amd64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
	  ...
	  GOOS=$$os GOARCH=$$arch CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$(BRIDGE_VERSION)" -o bin/k8s-bridge-$$os-$$arch$$ext . ; done
